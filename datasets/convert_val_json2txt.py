...
#from ...datasets.data_config import get_val_data
from datasets.data_config import get_val_data
...
def convert_annotation(ffile,image_id):
    out_file = open(get_val_data() + origin_label_folder + '/%s.txt' % (image_id), 'w')
    image = Image.open(os.path.join(get_val_data(),origin_img_folder,'%s.jpg' % (image_id)))
    w = image.width
    h = image.height
...
if __name__ == "__main__":
    current = 0
    with open(os.path.join(get_val_data(),origin_img_folder,"annotations_val.json")) as file:
        data = json.load(file)
